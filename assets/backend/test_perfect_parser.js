...
function parseCriteriaSheet(filePath, sheetName, specKey) {
  ...
  for (let r = 0; r < rows.length; r++) {
    ...
    // Nếu chưa có Domain thật sự (chưa gặp I.) thì bỏ qua các dòng tiêu đề phía trên
    if (!currentDomain) {
      continue;
    }
...
targetFiles.forEach(tf => {
  ...
  res.domains.forEach(d => {
    const dCritCount = d.standards.reduce((acc, s) => acc + s.criteria.length, 0);
    console.log(`   ${d.id} (${d.code}) ${d.name.substring(0, 35)}...: ${dCritCount} TC, ${d.standards.length} standards`);
  });
});
