...
function parseCriteriaSheet(filePath, sheetName, specKey) {
  ...
  for (let r = 0; r < rows.length; r++) {
    ...
    if (isDomain) {
      ...
      let maxPts = typeof col5 === 'number' ? col5 : (rows[r+1] && typeof rows[r+1][5] === 'number' ? rows[r+1][5] : (rows[r+1] && parseInt(rows[r+1][5])) || 200);
      currentDomain = {
        id: `${specKey}_d${dIdx++}`,
        code: dCode,
        name: dName,
    ...
    // 2. Kiểm tra Tiêu chuẩn (Standard)
    ...
    if (col1.toLowerCase().includes('tiêu chí') && (typeof col2 === 'number' || (col2 && !isNaN(parseInt(col2))))) {
      if (!currentStandard) {
        currentStandard = { id: `${specKey}_std_${sIdx++}`, number: sIdx - 1, title: `Tiêu chuẩn ${sIdx - 1}`, criteria: [] };
        currentDomain.standards.push(currentStandard);
      }

      const cNum = parseInt(col2);
      const optText = col3;
      const optDesc = col4;
      const score = typeof col5 === 'number' ? col5 : (parseInt(col5) || 0);

      // Nếu tiêu chí này đã tồn tại (do có 2 dòng tiêu chí cùng số như TC 19 ở GMHS, hoặc 32, 33, 35, 36 ở VLTL)
      if (critMap.has(cNum)) {
  ...
  // Chuẩn hóa lại level cho các options
  ...
  domains.forEach(d => {
    d.standards = d.standards.filter(s => s.criteria.length > 0);
  });

  return { domains, totalCriteria: critMap.size, maxNum: Math.max(...Array.from(critMap.keys())) };
}

module.exports = { parseCriteriaSheet, targetFiles, folderPath };
...
if (require.main === module) {
  ...
  targetFiles.forEach(tf => {
    ...
    if (res.totalCriteria !== tf.expected) {
      console.error(`❌ [${tf.key}]: Expected ${tf.expected} but got ${res.totalCriteria}`);
      allValid = false;
    } else {
      console.log(`✅ [${tf.key.toUpperCase()}]: Validated ${res.totalCriteria} TC across ${res.domains.length} domains.`);
    }
  });
  if (allValid) console.log('🎉 ALL 6 SPECIALTY DATASETS VALIDATED SUCCESSFULLY!');
}
