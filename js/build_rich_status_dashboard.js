...
// Ensure submission saving is 100% rock-solid in submitAssessmentForm
const oldSubmitExecFinal = `    saveStorage();
    if (typeof cloudSyncSubmission === 'function') cloudSyncSubmission(sub);`;

const newSubmitExecFinal = `    // Đảm bảo lưu trọn vẹn điểm số, minh chứng và lịch sử nộp lên hệ thống
...
if (content.includes(oldSubmitExecFinal)) {
  content = content.replace(oldSubmitExecFinal, newSubmitExecFinal);
}

// Ensure manager approval in modal copies adjusted scores to official score
const oldApproveLevelUpdates = `  if (level === 1) {
...
const newApproveLevelUpdates = `  // Đồng bộ điểm điều chỉnh nếu quản lý có thay đổi điểm
...
if (content.includes(oldApproveLevelUpdates)) {
  content = content.replace(oldApproveLevelUpdates, newApproveLevelUpdates);
}

// Build the enhanced renderStatusTab with Timeline & Dashboard
const newStatusTabCode = `
// =========================================================
...
function renderStatusTab() {
  ...
  const timelineSteps = [
    ...
    {
      num: 5,
      title: 'Phê Duyệt Cấp 4: Ban Giám Đốc',
      subtitle: 'Quyết định công nhận & vinh danh',
      role: 'Ban Giám Đốc Bệnh Viện',
      actor: sub.l4ApprovedBy || 'PGS.TS.BS Hà Mạnh Tuấn',
      time: sub.approvedAt ? new Date(sub.approvedAt).toLocaleString('vi-VN') : (status === 'submitted_l4' ? 'Đang phê duyệt' : (status === 'approved' ? 'Đã ban hành QĐ' : 'Chờ trình')),
      state: status === 'approved' ? 'completed' : (status === 'submitted_l4' ? 'active' : 'pending'),
      icon: status === 'approved' ? '👑' : (status === 'submitted_l4' ? '🏛️' : '5'),
      desc: status === 'approved' 
        ? \`Quyết định số \${sub.decisionNo || '089/QĐ-BVĐHYD-CS2/2026'} đã ban hành chính thức.\`
  ...
  container.innerHTML = \`
    <div class="space-y-5">
      ...
      <!-- Grand Honor Showcase if Approved -->
      \${grandHonorCardHtml}
      ...
      <div class="bg-white rounded-3xl border border-slate-200 p-5 sm:p-6 shadow-sm space-y-5">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
          ...
          <div class="p-4 rounded-2xl bg-gradient-to-br from-blue-700 to-indigo-800 text-white shadow-sm space-y-2 flex flex-col justify-between">
            ...
            <div>
              ...
              <div class="w-full h-2 bg-blue-950/40 rounded-full overflow-hidden">
                <div class="h-full bg-amber-400 rounded-full" style="width: \${scorePct}%"></div>
              </div>
            </div>
          </div>

          <!-- KPI 2 -->
          <div class="p-4 rounded-2xl bg-gradient-to-br from-amber-500 to-yellow-600 text-slate-950 shadow-sm space-y-2 flex flex-col justify-between">
            <div>
              <div class="text-[11px] font-black text-amber-950 uppercase tracking-wide">Bậc Năng Lực Đạt Được</div>
              <div class="text-xl sm:text-2xl font-black mt-1">\${tierObj.name}</div>
              <div class="text-xs font-bold text-amber-900">\${tierObj.title}</div>
            </div>
            <div class="text-[11px] bg-white/70 px-2 py-1 rounded-lg border border-amber-400/50 font-bold">
              Đạt chuẩn quy định BV UMC
            </div>
          </div>
          ...
          <div class="p-4 rounded-2xl bg-gradient-to-br from-emerald-600 to-teal-700 text-white shadow-sm space-y-2 flex flex-col justify-between">
            <div>
              <div class="text-[11px] font-bold text-emerald-200 uppercase tracking-wide">Tiêu Chí Đã Đánh Giá</div>
              <div class="text-2xl sm:text-3xl font-black mt-1">\${scoredCritCount} <span class="text-sm text-emerald-200 font-normal">/ \${totalCritCount} TC</span></div>
            </div>
            <div class="text-[11px] text-emerald-100 flex items-center justify-between">
              <span>Độ bao phủ:</span>
              <strong class="font-black text-white">\${completionPct}%</strong>
            </div>
          </div>

          <!-- KPI 4 -->
          <div class="p-4 rounded-2xl bg-gradient-to-br from-purple-700 to-indigo-900 text-white shadow-sm space-y-2 flex flex-col justify-between">
            <div>
              <div class="text-[11px] font-bold text-purple-200 uppercase tracking-wide">Hồ Sơ Minh Chứng Số</div>
              <div class="text-2xl sm:text-3xl font-black mt-1">\${evidenceCount} <span class="text-sm text-purple-200 font-normal">Tệp đính kèm</span></div>
            </div>
            <div class="text-[11px] text-purple-100 flex items-center justify-between">
              <span>Trạng thái:</span>
              <strong class="font-black text-white">✓ Đã đối chiếu số hóa</strong>
            </div>
          </div>
        </div>

        <!-- 5 Domains Breakdown Cards -->
        <div class="space-y-3 pt-2">
          <div class="text-xs font-black uppercase text-slate-700 flex items-center justify-between">
            <span>PHÂN BỐ ĐIỂM SỐ THEO 5 LĨNH VỰC NĂNG LỰC CỐT LÕI</span>
            <span class="text-[11px] text-slate-400 font-normal">Chuẩn hóa theo khung năng lực 7 bậc</span>
          </div>
        ...
        <!-- So Sánh Điểm Số Qua 4 Cấp Thẩm Định -->
        <div class="pt-4 border-t border-slate-100">
          <div class="text-xs font-black uppercase text-slate-700 mb-3 flex items-center gap-2">
            <span>📈</span> BẢNG ĐỐI CHIẾU ĐIỂM SỐ QUA 4 CẤP THẨM ĐỊNH
          </div>

          <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
            <div class="p-3 bg-slate-50 rounded-xl border border-slate-200">
              <div class="text-[10px] text-slate-500 uppercase font-bold">1. Nhân Viên Tự Chấm</div>
              <div class="text-base font-black text-slate-900 mt-1">\${sub.selfScore || sub.totalScore || 0}đ</div>
              <div class="text-[10px] text-slate-500 mt-0.5">Hoàn tất tự chấm</div>
            </div>
            <div class="p-3 bg-slate-50 rounded-xl border border-slate-200">
              <div class="text-[10px] text-slate-500 uppercase font-bold">2. ĐD Trưởng Duyệt C1</div>
              <div class="text-base font-black text-blue-900 mt-1">\${sub.l1Score !== undefined ? sub.l1Score : (sub.selfScore || sub.totalScore || 0)}đ</div>
              <div class="text-[10px] text-emerald-700 font-bold mt-0.5">✓ Đạt yêu cầu</div>
            </div>
            <div class="p-3 bg-slate-50 rounded-xl border border-slate-200">
              <div class="text-[10px] text-slate-500 uppercase font-bold">3. Trưởng Khoa Duyệt C2</div>
              <div class="text-base font-black text-sky-900 mt-1">\${sub.l2Score !== undefined ? sub.l2Score : (sub.l1Score || sub.selfScore || sub.totalScore || 0)}đ</div>
              <div class="text-[10px] text-emerald-700 font-bold mt-0.5">✓ Chuẩn y</div>
            </div>
            <div class="p-3 bg-slate-50 rounded-xl border border-slate-200">
              <div class="text-[10px] text-slate-500 uppercase font-bold">4. Ban ĐD & BGĐ (C3 & C4)</div>
              <div class="text-base font-black text-purple-900 mt-1">\${sub.totalScore || 0}đ</div>
              <div class="text-[10px] text-purple-700 font-black mt-0.5">👑 ĐÃ CÔNG NHẬN</div>
            </div>
          </div>
        </div>
...
const oldStatusTabFuncStart = "function linkCertificateToPortfolio(userId, year) {";
const oldStatusTabFuncEnd = "function viewOfficialDecision(userId) {";

if (content.includes(oldStatusTabFuncStart) && content.includes(oldStatusTabFuncEnd)) {
  const startIdx = content.indexOf(oldStatusTabFuncStart);
  const endIdx = content.indexOf(oldStatusTabFuncEnd);
  content = content.slice(0, startIdx) + newStatusTabCode.trim() + "\n\n" + content.slice(endIdx);
}

fs.writeFileSync('app.js', content, 'utf8');
fs.writeFileSync('js/app.js', content, 'utf8');

console.log('Successfully updated app.js and js/app.js with timeline & score dashboard!');
