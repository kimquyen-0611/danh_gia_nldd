...
console.log('--- ĐỒNG BỘ TOÀN DIỆN CÁC TỆP JS ---');

// 1. UPDATE KHOA KHÁM BỆNH TIÊU CHÍ 31 -> -20đ
function fixCriteria31(content) {
...
// 2. UPDATE SUPABASE CLIENT TO SANITIZE PAYLOAD (PREVENT 400 ERROR)
const safeSupabaseCode = `/**
...
// Ghi đè vào cả root và js/
fs.writeFileSync('supabase_client.js', safeSupabaseCode, 'utf8');
fs.writeFileSync('js/supabase_client.js', safeSupabaseCode, 'utf8');
console.log('✅ Đã cập nhật supabase_client.js và js/supabase_client.js');

// 3. Cập nhật và đồng bộ app.js và js/app.js
let appContent = fs.readFileSync('js/app.js', 'utf8');
appContent = fixCriteria31(appContent);
fs.writeFileSync('js/app.js', appContent, 'utf8');
fs.writeFileSync('app.js', appContent, 'utf8');
console.log('✅ Đã đồng bộ app.js và js/app.js');

// 4. Ẩn badge Backend (5000) trong index.html
let html = fs.readFileSync('index.html', 'utf8');
html = html.replace(/<div id="cloud-sync-container"[\s\S]*?<\/div>/, '<div id="cloud-sync-container" class="hidden"></div>');
fs.writeFileSync('index.html', html, 'utf8');
console.log('✅ Đã ẩn hoàn toàn chữ Backend (5000) trong index.html');
